...
const refreshClinicRating = async (clinicId, tenantId) => {
  const [summary] = await Review.aggregate([
    { $match: { clinic: new mongoose.Types.ObjectId(clinicId), tenantId: new mongoose.Types.ObjectId(tenantId) } },
    { $group: { _id: "$clinic", rating: { $avg: "$rating" }, reviews: { $sum: 1 } } },
  ]);
  await Clinic.findOneAndUpdate({ _id: clinicId, tenantId }, { rating: summary?.rating ? Number(summary.rating.toFixed(1)) : 0, reviews: summary?.reviews || 0 });
};
...
export const listClinicReviews = async (req, res) => {
  ...
  try { return res.json(await reviewQuery(Review.find({ clinic: req.params.clinicId, tenantId: req.tenantId }))); }
...
export const listUserReviews = async (req, res) => {
  try { return res.json(await reviewQuery(Review.find({ user: req.user._id, tenantId: req.tenantId }))); }
...
export const createReview = async (req, res) => {
  ...
  if (!appointment || invalidId(appointment) || !Number.isInteger(Number(rating)) || Number(rating) < 1 || Number(rating) > 5) return res.status(400).json({ message: "A completed appointment and rating from 1 to 5 are required" });
  try {
    const completedAppointment = await Appointment.findOne({ _id: appointment, user: req.user._id, tenantId: req.tenantId, status: "completed" });
    if (!completedAppointment) return res.status(400).json({ message: "Reviews are available after a completed appointment" });
    const existingReview = await Review.findOne({ appointment, tenantId: req.tenantId });
...
export const deleteReview = async (req, res) => {
  if (invalidId(req.params.reviewId)) return res.status(404).json({ message: "Review not found" });
  try {
    const review = await Review.findOneAndDelete({ _id: req.params.reviewId, user: req.user._id, tenantId: req.tenantId });
    if (!review) return res.status(404).json({ message: "Review not found" });
    await refreshClinicRating(review.clinic, req.tenantId);
