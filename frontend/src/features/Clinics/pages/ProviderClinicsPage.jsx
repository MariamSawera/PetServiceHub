  ...
  return (
    <main className="min-h-[70vh] bg-[var(--theme-bg)] px-6 py-12 md:px-12 md:py-16">
      <div className="mx-auto max-w-[1400px]">
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-indigo-700">Provider workspace</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm md:p-8">
          <div className="flex items-center gap-3"><Building2 className="text-indigo-600" /><h2 className="text-xl font-bold text-slate-950">{editingId ? 'Edit clinic' : 'Create clinic'}</h2></div>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {[['name', 'Clinic name', true], ['city', 'City', true], ['address', 'Address', false], ['phone', 'Phone', false], ['specialty', 'Primary specialty', false]].map(([field, label, required]) => <label key={field} className="text-sm font-semibold text-slate-700">{label}<input required={required} type="text" value={form[field]} onChange={handleChange(field)} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 font-normal outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" /></label>)}
            <label className="text-sm font-semibold text-slate-700 md:col-span-2">Description<textarea value={form.description} onChange={handleChange('description')} rows="3" className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 font-normal outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" /></label>
          </div>
          ...
          <details className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><summary className="cursor-pointer text-sm font-bold text-slate-700">Advanced location</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Latitude<input type="number" step="any" value={form.latitude} onChange={handleChange('latitude')} className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 font-normal outline-none focus:border-indigo-500" /></label><label className="text-sm font-semibold text-slate-700">Longitude<input type="number" step="any" value={form.longitude} onChange={handleChange('longitude')} className="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 font-normal outline-none focus:border-indigo-500" /></label></div></details>
          {message && <p className="mt-4 text-sm font-semibold text-indigo-700">{message}</p>}
          <div className="mt-6 flex flex-wrap gap-3"><button type="submit" className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-3 text-sm font-bold text-white hover:bg-indigo-700"><Plus size={17} />{editingId ? 'Save changes' : 'Create clinic'}</button>{editingId && <button type="button" onClick={() => { setEditingId(null); setForm(EMPTY_FORM); }} className="rounded-lg border border-slate-200 px-5 py-3 text-sm font-bold text-slate-700">Cancel</button>}</div>
        </form>

        {state === 'error' && <p className="mt-8 rounded-xl bg-red-50 p-5 text-sm font-semibold text-red-700">Could not load your clinics.</p>}
        {state === 'ready' && clinics.length === 0 && <p className="mt-8 rounded-2xl border border-dashed border-indigo-200 bg-white p-12 text-center text-sm text-slate-500">No clinics yet. Create your first clinic above.</p>}
        {state === 'ready' && clinics.length > 0 && <div className="mt-8 grid gap-4 md:grid-cols-2">{clinics.map((clinic) => <article key={clinic._id} className="flex items-start justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div><h2 className="font-bold text-slate-950">{clinic.name}</h2><p className="mt-1 text-sm text-slate-500">{clinic.city}</p><p className="mt-2 text-xs text-indigo-700">Owner: your provider account</p></div><div className="flex gap-2"><button type="button" onClick={() => handleEdit(clinic)} aria-label={`Edit ${clinic.name}`} className="rounded-lg p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700"><Pencil size={17} /></button><button type="button" onClick={() => handleDelete(clinic._id)} aria-label={`Delete ${clinic.name}`} className="rounded-lg p-2 text-slate-500 hover:bg-red-50 hover:text-red-600"><Trash2 size={17} /></button></div></article>)}</div>}
      </div>
    </main>
  );
}
